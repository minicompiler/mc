...
| `mc: mc.lock is stale: geo` | 2 | `[deps]` names a package the lock lacks, or a version constraint the version the lock pins does not satisfy (a raised minimum, a tightened `~`/`^` ceiling, a new `=` pin). With no name after the colon, the lock file itself is missing | `mc pkg sync --yes` |
