...
| `on_source handler pushed a source: <name>` | the hook's one guard: a handler pushed a source from inside the callback (`p_push_source`, `lex_include`, an `#include` it triggered). The detail is the source it tried to push. Without the guard a handler that pushes unconditionally recurses until the process stack is gone | read the buffer, do not open another one; push what you have to push after `user_init` returns, or from a `syntax`/`syntax_stmt` handler |
| `source_claim handler pushed a source: <name>` | the same guard on the other lexer hook: a `source_claim` handler pushed a source from inside the callback. A claim is answered from the name alone, and a push there interleaves one frame with the next | answer from `name`; open nothing |
...
| `p_skip_to outside the source token` | a handler said it had consumed bytes up to a point before the cursor, past the end of the source, or on a token that was not just lexed from the source being read | `p_take_lit`'s guard exactly: never a string, never a substituted identifier |
...
| `opcode <n> is not arm64's: no machine claims it` / `opcode <n> is not x86-64's: no machine claims it` | the bundled machine was handed an opcode above its own `I_COUNT` / `X_COUNT`. A derived machine claims a BAND and passes everything else down ([machine.md](machine.md) § The opcode bands), so this is a band whose owner is not in the compiler that is running — typically a module whose `intrinsic` registrations outlive its machine, e.g. `<f16>` (arm64 only) reached through `--backend=elf-obj-x86_64` or `--machine=x86_64`, both of which are read after `user_init` and so cannot be seen by the module | name a target the module drives: `mc build` with `[target]`, which is read before `user_init` |
| `machine answers MTASK_REG_COUNT but leaves a version 5 slot empty` | a machine offered the walker allocatable registers (`MTASK_REG_COUNT` non-zero) and left one of `REG_LOAD`/`REG_STORE`/`REG_SAVE`/`REG_RESTORE`/`PARAM_REG` at 0. The null-slot rule is all-or-nothing: a 0 count means "no allocatable registers", but a non-zero one is a promise to answer the other five ([machine.md](machine.md) § 5) | fill the six, or answer 0 |
| `--opt must be 0 or 1: <value>` | `--opt=` on the command line, or `mc build --opt=`, with anything but `0` or `1`. There is one optimized road, not three levels ([cli.md](cli.md)) | `--opt=1`, or `-O` |
| `<file>: must be 0 or 1: project.opt` | the same value written in `mc.toml` | as above |
...
| `--yes applies to --sync: mc build downloads nothing without it` | `mc build --yes` with no `--sync`. `--yes` is read by the sync step and by nothing else, so on its own it is a flag nobody reads — refused rather than ignored, the post-M42 rule | add `--sync`, or drop `--yes` |
| `--sync needs the package half of this compiler: mc pkg is not in it` | `mc build --sync` on a compiler assembled without `<mc/core_pkg>`. The sync step is registered by that part as a pointer, and there is none. `--sync` is `mc build`'s flag, so the usage still lists it: a subcommand disappears with its part ([bundle.md](bundle.md) § The parts), a flag inside one says what is missing | use a compiler that has the part (`mc` itself, or `mc-slim`), or sync separately with one and build with this |
...
| `<os>/<arch> requires a linker: there is no direct executable` | `--exe` on a host whose `target()` registration has 0 in the exe slot. `macos` and `linux` (both arches) have a direct executable; on Windows it is per-arch: **windows/x86_64** has one (`pe-exe-x86_64`), **windows/aarch64** does not (an arm64 PE needs `DYNAMICBASE` + a `.reloc` base-relocation table validated on real Windows-on-ARM, deferred), so `mc --exe` on a windows/aarch64 host prints `windows/aarch64 requires a linker: there is no direct executable`. It also applies to any pair a module registers with a 0 exe slot | compile to an object (`mc x.mc -o x.o`) and link it, or use `mc build` with a `[linker]` section. This is the single-file CLI's wording of the driver's `<os>/<arch> requires [linker]: there is no direct executable` (§ 10): same registration, same refusal, and `[linker]` is a TOML section a command line does not have |
...
| `<path> exited <N>` / `<path> killed by signal <N>` | 0.16.1: a tool the driver spawned — the `[linker]`, or the taught compiler `mc build` runs for the second half — failed with a status other than 1. **Exit 1 is exempt**: that is how every diagnostic in this compiler ends and how a linker reports its own error, so the tool has already said what was wrong and a second line here would only push it off the end of the output. Anything else is a death the tool had no chance to report, and 127 from a loader that refused the executable is the case that asked for it — before this the build printed its `compiler x.mc -> x` step line and stopped with exit 1 and nothing else (`docs/specs/M42.md` § Implementation notes). 128 + N is read as a signal, the shell's convention | run the named program by hand: a compiler that dies before its first instruction is a link or a loader problem, not a source one |
...
| `<os>/<arch> requires [linker]: there is no direct executable` | `target.os` | the pair is registered and its **executable** slot is 0, which is a registration saying this target has no direct executable at all. `macos` and `linux` (both arches) have one; on Windows it is per-arch — **windows/x86_64** has an exe backend (`pe-exe-x86_64`), **windows/aarch64** does not (an arm64 PE is deferred: it needs `DYNAMICBASE` + a `.reloc` table validated on real Windows-on-ARM), so `mc build` for windows/aarch64 with `kind = "exe"` and no `[linker]` prints `windows/aarch64 requires [linker]: there is no direct executable`. It also applies to any pair a module registers with `target(os, arch, obj, 0)`. Add a `[linker]` section, or `kind = "obj"` and link the object yourself |
| `no main and no mc_start: cannot generate an executable` | — | a `pe-exe-x86_64` (Windows PE) executable was asked for from a translation unit that defines neither `main` nor `mc_start`. The writer's synthesized entry needs `main`; a program that brings its own entry defines `mc_start` (`#include <sys_windows_start>`). Define one of them |
...
| `mc: deps/geo is 1.0.0, mc.lock wants 1.2.0: update the checkout or remove deps/geo` | 2 | a VENDORED tree that declares `[package].version`, and declares a version other than the one that was resolved -- a checkout left behind while `[deps]` moved. Asked before the tree is hashed, so what would otherwise be a `checksum mismatch` on bytes that are not corrupt is this sentence instead. The requirement is named where it was read: `mc.lock` on `mc build`/`verify`/`list`, `[deps]` on `mc pkg sync` | update the checkout, or delete `deps/geo` and let the cache answer |
| `deps/geo/mc.toml:3:11: not a usable version` | 2 | a `[package].version` that is not `[0-9A-Za-z.+_-]`, or carries a `..` | it becomes a comparison and never a path, but it is checked like every other version |
| `mc: mc.lock is stale: geo` | 2 | `[deps]`/`[tools]` names a package the lock lacks, or a version constraint the version the lock pins does not satisfy (a raised minimum, a tightened `~`/`^` ceiling, a new `=` pin). With no name after the colon, the lock file itself is missing | `mc pkg sync --yes` |
| `mc: mathx: no version satisfies: >= 1.1.0 < 1.1.0` | 2 | the version constraints on one package have an empty intersection, or no registered version falls in the range; the bounds shown are the accumulated `>= low < high` | loosen a constraint, or add a version to the registry |
| `mc: mathx: two exact pins: =1.0.0 and =1.1.0` | 2 | two `=X.Y.Z` constraints on one package name different versions | one exact pin per package |
| `mc.toml:8:9: a version constraint must be X.Y.Z, =X.Y.Z, ~X.Y.Z, ^X.Y.Z, >=X.Y.Z, or *` | 2 | a `[deps]`/`[tools]` value that is none of the six forms -- a `>`/`<`/`<=`, an `X.Y` short version, a word. On a registry row it is `malformed version constraint in a registry dep` (exit 2, no position) | use one of the six forms; a ceiling comes from `~`/`^`, never a bare `<` |
...
| `geo 1.2.0 needs mc >= 1.2.0 (this is mc 1.1.0): upgrade the compiler` | 2 | the running compiler does not satisfy a package's `[package].mc` constraint. The phrase is `>= <min>` for a bare/`>=` minimum, `>= <min> and < <max>` for a capped `~`/`^`, or `= <exact>` for a `=` pin. At the key's own `file:line:col`, naming the entry by its `[package].name` or a dependency by its locked name-and-version. A `0.0.0-dev` working-tree build is treated as satisfying every constraint and never raises it ([packages.md](packages.md) § 3) | install a matching mc, or relax the constraint if it was set too tight |
| `mc.toml:3:6: package.mc must be a version like 1.2.3, ">= 1.2.3", "^1.2.3" or "=1.2.3"` | 2 | a `[package].mc` value that is not one of the version-constraint forms; a bare range or a word. Validated before the version compare, so it is raised on every compiler including a dev build | a bare/`>=` is the normal (minimum) form; `^`/`~`/`=` caps the compiler version |
...
| `mc: geo 1.9.0: no such version in the registry` | 1 | a `[deps]` minimum, or an `@VERSION`, that no index row carries. On a URL registry it is raised only after the snapshot has been refreshed, and only with `--yes`: without one the command prints the index-fetch plan instead, since a snapshot older than the registry cannot answer this ([packages.md](packages.md) § 10) | `mc pkg list`, or the index page |
