...
| `<os> requires a linker: there is no direct executable` | post-M41 review: `--exe` on a host whose `target()` registration has 0 in the exe slot — a pair a module registers that way. No target the core registers has a 0 exe slot any more: `linux` gained one at M42, `windows` at M42 step 2. `linux` and `windows` were on this list until then | compile to an object (`mc x.mc -o x.o`) and link it, or use `mc build` with a `[linker]` section. This is the single-file CLI's wording of the driver's `<os> requires [linker]: there is no direct executable` (§ 10): same registration, same refusal, and `[linker]` is a TOML section a command line does not have |
...
| `<os> requires [linker]: there is no direct executable` | `target.os` | the pair is registered and its **executable** slot is 0, which is a registration saying this target has no direct executable at all. No pair the core registers is in this state now: `linux` gained an exe backend at M42 (`elf-exe`) and `windows` at M42 step 2 (`pe-exe-arm64`/`pe-exe-x86_64`). It applies only to a pair a module registers with `target(os, arch, obj, 0)`. Add a `[linker]` section, or `kind = "obj"` and link the object yourself |
| `no main and no mc_start: cannot generate an executable` | — | a `pe-exe-*` (Windows PE) executable was asked for from a translation unit that defines neither `main` nor `mc_start`. The writer's synthesized entry needs `main`; a program that brings its own entry defines `mc_start` (`#include <sys_windows_start>`). Define one of them |
