...
| `host_self_path()` | the path of the file this process was loaded from, or 0 when the host cannot say. `_NSGetExecutablePath` on macOS, `readlink("/proc/self/exe")` on Linux, `GetModuleFileNameA(0, ...)` on Windows -- never `argv[0]`, which is whatever the caller passed and is a bare name after a `PATH` lookup. The answer is not resolved any further: a process started through a symlink is told the file the kernel loaded. The callers are `mc upgrade` ([cli.md](cli.md) § 3f), which replaces exactly that file (M44 step 5), and `mc tool` (M48 C3), whose install builds the tool by spawning this file and whose launcher names it |
