...
| function | effect |
|---|---|
| `void lex_set_libs(uptr openfn, uptr hintfn)` | register the opener `#include <name>` consults BEFORE the bundle and, on a bundle miss, after it, plus a diagnostic hint hook. `openfn` has the bundle's signature plus a stage: `uptr f(uptr name, i64 stage, uptr pcanon, uptr plen)`, `stage == 0` the lock road and `stage == 1` the installed `mc` package; it returns the source or 0, and on a hit writes the once-only key through `pcanon` and the length through `plen`. `hintfn` is `uptr g(uptr name)`, consulted only by a compiler with NO bundle at all (`bopen_fn == 0`, the slim flavour) when an `<name>` include misses: it returns a hint string for the diagnostic (e.g. name the `mc` package to install) or 0 for the plain `unknown bundled include`. Pass `0` for `hintfn` to keep the plain message. `src/core_build.mc` registers `libs_open` (and, for the slim binary, `dep_include_hint`) here, which is why a compiler assembled without `<mc/core_build>` resolves `<name>` from its bundle alone. (Since 0.15.18 -- PR #50, the second argument; a one-argument call is `wrong number of arguments`.) |
...
| `host_which(name)` | the first executable called `name` on this host's `PATH`, as an absolute path, or 0 when there is none; a `name` that already contains a slash is a path and is answered as one, which is what `execvp` does. It is in the host layer and not beside its caller because the environment is the host's and so is the separator that splits `PATH` (`:` here, `;` on Windows). The one caller is `mc sandbox --bin PROG` ([sandbox.md](sandbox.md) § The primitives), which resolves it in P before any `unshare` — once the box has pivoted, the host's `PATH` names nothing. `src/host_macos.mc` and `src/host_windows.mc` answer 0 and say why: there is no sandbox on either, so the only caller cannot reach them. A candidate is tested with `c_int(open(...)) >= 0` and not with the raw result: `open` returns a C `int`, and on glibc 2.39 a failing one comes back as `0x00000000ffffffff`, which a 64-bit test reads as success — without `c_int` this function answered the first entry of `PATH` with the name appended, existing or not (see [language.md](language.md) § 6 and [../specs/M43.md](../specs/M43.md) § the C2 primitives, note 10) |
...
| `host_self_path()` | the path of the file this process was loaded from, or 0 when the host cannot say. `_NSGetExecutablePath` on macOS, `readlink("/proc/self/exe")` on Linux, `GetModuleFileNameA(0, ...)` on Windows -- never `argv[0]`, which is whatever the caller passed and is a bare name after a `PATH` lookup. **A symlink is resolved**: a process started through `~/bin/mc -> .../build/mc1` is told the second path, so the library roots derived from it (2 and 3, [packages.md](packages.md) § 2) are the ones beside the real binary, and `mc upgrade` replaces the compiler rather than deleting the link. On Linux and Windows the system answers that way already (`/proc/self/exe` is the resolved target; `GetModuleFileNameA` resolves too); on macOS `_NSGetExecutablePath` answers the path as invoked and the host layer passes it through `realpath(3)`, falling back to the raw path if that fails. The callers are `mc upgrade` ([cli.md](cli.md) § 3f), which replaces exactly that file (M44 step 5), `mc tool` (M48 C3), whose install builds the tool by spawning this file and whose launcher names it, and the library search |
| `host_getcwd()` | the current working directory of the invocation, absolute, or 0 when the host cannot say. `getcwd` on macOS and Linux, `GetCurrentDirectoryA` on Windows. The one caller is `mc tool` (M48 C3): it is what the `workspace` permission resolves against ([tools.md](tools.md) § The workspace) and what `mc tool box-args` prints, so `mc tool run` and a registry validator agree on where the box's read root is |
...
| `host_audit_arch()` | the `AUDIT_ARCH_*` value the seccomp filter compares against before it looks at a system call number -- `0xC00000B7` on AArch64, `0xC000003E` on x86-64, 0 where there is no seccomp. It is a host answer and not a constant in `src/seccomp.mc` for the same reason the number table is: a value that changes with the architecture belongs to the host layer (M43) |
...
`host_syscall6`, `host_sysno`, `host_audit_arch` and `host_sandbox_supported` are the M43
additions, and they follow
...
Ten registrations, eight of them in `src/hooks.mc`, all called from `user_init()` (or,
