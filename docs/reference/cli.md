...
| `--exe` | write a direct executable for the HOST, no linker. The backend is the exe slot of the host's `target()` registration, resolved after `user_init()` (post-M41 review) — `macho-exe` on macOS, a signed Mach-O binary, and `elf-exe` / `elf-exe-x86_64` on Linux, a dynamic ELF64 `ET_EXEC` (M42) whose `PT_INTERP` names **musl's** loader unless `--libc=gnu` says otherwise. A host registered with 0 in that slot has no direct executable at all, and the flag is refused with `<os>/<arch> requires a linker: there is no direct executable` instead of writing a binary for another operating system. On Windows this is per-target: **windows/x86_64** has a direct executable (`pe-exe-x86_64`), so `mc --exe` on that host writes a PE; **windows/aarch64** has none — an arm64 PE needs `DYNAMICBASE` + a `.reloc` base-relocation table validated on real Windows-on-ARM (deferred) — so its exe slot is 0 and `mc --exe` there is refused with the same message, `windows/aarch64 requires a linker: there is no direct executable`. A Windows x86_64 PE is cross-built with `--backend=pe-exe-x86_64` or by `mc build` with `kind = "exe"` and no `[linker]`; windows/aarch64 goes through `[linker]` (object + `lld-link`). `--exe` and `--backend=` write the same decision, so the last one on the command line wins. |
...
unknown backend: xyz
registered: macho macho-exe elf-obj elf-obj-x86_64 elf-exe elf-exe-x86_64 coff-obj-arm64 coff-obj-x86_64 pe-exe-x86_64
```
