...
Repository `minicompiler/mc-registry`:
...
`https://pkg.minicompiler.dev` by default (`pkg_default_registry()` in `src/pkg.mc` -- the
draft said `raw.githubusercontent.com/minicompiler/mc-registry/main`; see § Implementation notes
4, Deviations), `[registry].url` in `mc.toml`, or `--registry URL|DIR`. An `https://` registry is
...
| `minicompiler/mc-registry` (separate repository) | `README.md` ~80, `.github/workflows/check.yml` ~70, `index/` | seeded with the owner's first package; the teko port when it exists |
| `stage0/`, `lib/` | 0 | |
...
* **a release asset** (default for the SLIM binary, which has no blob to read):
  `https://github.com/minicompiler/mc/releases/download/v<ver>/mc-libs-<ver>.tar.gz` and its `.sha256`,
...
1. **Which version.** With no `VERSION`: fetch
   `https://github.com/minicompiler/mc/releases/latest/download/LATEST` -- GitHub resolves
...
the owner. Anyone who can serve `github.com/minicompiler/mc/releases/...` to this machine can serve a
...
| `minicompiler/mc-registry` | as in the draft | `check.yml` refuses `name = "mc"` and any `mc/...` |
| `stage0/` | 0 | |
...
**4. Two path bugs that only a real fixture finds**, both worth writing down because they were
silent:

* `path_norm` **drops a trailing slash**, so `path_join(cfg, "deps/geo/")` came back as
  `.../deps/geo` and the next join cut it off as if it were a file name — the vendored road looked
  for `deps/mc.toml`. The directory is now normalised once, in `dp_set_dir`, and the `/` is
  re-attached after.
* macOS `TMPDIR` ends in `/`, so `--libs-dir "$TMPDIR/x/libs"` carries a `//`. The package roots
  are compared against paths that came out of `path_join`, which normalises, so the root was a
  prefix of nothing: `lex_root_of` answered -1 for every file and **the closure rule silently never
  fired**. Same fix, same line. Both are the reason `dp_set_dir` normalises rather than storing
  what it was given.
...
* **The per-file attribution needs the cache manifest, and a vendored tree has none.** The lock
  ...
  the file is `<libs>/<pack>/v<version>.toml`, the `[[file]]` list the fetch writes last. The draft
  § 3 table assumes it. A vendored tree has no manifest, so its refusal is
  `geo 1.2.0: the tree does not match mc.lock` — a third row the draft did not have. The reader is
  ~40 lines and it was implemented now rather than in step 3, because the manifest is also what
  makes `is not fetched` exact: a half-extracted directory has no manifest and is reported as *not
  fetched* instead of as *does not match*.
* **`mc.lock is stale` carries the M25 `run:` block**, not the draft's inline
  `mc.lock is stale: run mc pkg sync --yes`. Every exit-2 refusal in this compiler has the same
  shape (`src/sysroot.mc`, `docs/reference/cli.md` § Exit codes) and having one of them spell `run`
  inside its own first line would be the odd one out. The text is `mc: mc.lock is stale: geo` plus
  `  run:   mc pkg sync --yes`.
* **The once-only key for a file served from disk is its NORMALISED path, not its absolute path.**
  `mc` has no `getcwd` and does not add one for this: what matters is that the key is the same one
  `lex_include` would record for the same file, and both go through `path_join`/`path_norm`. It is
  absolute exactly when the config's path is.
* **`libs_open` re-attaches the `.mc` the lexer stripped.** `<geo/geo.mc>` and `<geo/geo>` are one
  name (A2), but a bundled name never carries an extension while a file on disk always does, so the
  disk road tries `rest` and then `rest + ".mc"`. That also leaves `<pack/table.txt>` — an `#embed`
  payload — findable under the name it was written with.
* **No arena tag for either new table.** The package roots and the edge list are sized ONCE, from
  the lock, before the first root is registered (`lex_pkg_reserve`); the per-package file list
  doubles inside `deps.mc`. None of the three scales with the program, and a growth event on a
  table whose size the developer wrote in `mc.lock` carries nothing anybody could act on — the M17
  argument for `MAXTARGETS`, with an exact count instead of a ceiling. `mc limits` therefore gains
  no row and `arena.mc` is untouched.
* **`[registry]` is read and unused.** `deps_registry()` exists so the key is validated and
  documented; nothing in `<mc/core_build>` reaches the network.
...
* **The default registry is `https://pkg.minicompiler.dev`**, not
  `raw.githubusercontent.com/minicompiler/mc-registry`. The owner decided (2026-09-04) that a package
  SERVER at that host will produce the § 5 layout — `index/<name>.toml`, immutable rows — out of
  the git repositories registered with it. The compiler gains nothing for it: the reader is the
  same file-at-a-time reader a directory registry uses, and the constant is one function,
  `pkg_default_registry()`. Everything the spec says about a tap still describes the FORMAT.
  **Corrected 2026-09-06**: the constant said `https://minicompiler.dev/registry` through 0.15.5,
  and the deployed server serves the index at `https://pkg.minicompiler.dev/index/<name>.toml` --
  so `mc pkg sync|add|check` with no `--registry` and no `[registry].url` resolved nothing. The
  registry moved first (`minicompiler/mc-registry` PR #11: the site host answers
  `/registry/index/<name>.toml` with the registry host's own bytes, an alias and not a second
  layout), and this constant moved second, to the canonical host -- the one the rows' own `url`
  and archive addresses already point at. A compiler older than 0.15.6 keeps working through the
  alias.
* **`mc pkg update` is `mc update`** (D21), and it stays inside its major. `go get -u` does not
  cross a major either, and raising a minimum across one is exactly the case D6 refuses to solve —
  so `mc update` would otherwise be a command whose whole job is to produce
  `different majors: no solver`. `mc pkg add NAME` with no `@` still takes the newest non-yanked
  row of any major: a name nothing requires yet has no major to stay inside of.
* **`sync` with nothing to download does not need `--yes`.** The plan is about downloads; when the
  build list is already on the disk, `sync` writes the lock and says so. With anything to fetch it
  prints the plan and stops, which is D13 as written.
* **A `[replace]`d package gets a `path` row and no `sha256`**, and `sync` prints
  `replaced geo: ../geo -- not pinned by mc.lock` — the same sentence `mc build` prints, from a
  different file. `src/deps.mc` needed no change for it: a row with no hash is not checked.
* **`check` compares against the registry's published copy**, which is the only way "a row never
  changes" can be enforced by a program that is handed one file. With a DIR registry the
  comparison is free; with a URL one it needs `--yes` (there is nothing to compare against
  otherwise) and it is skipped, loudly, by doing nothing.
* **`check` does not cross `[package].files` against `tar -t`** (§ Risks 7, priced as optional).
  What it does compare is the tree hash, the package name and the set of `[deps]`, which is what a
  wrong row actually gets wrong.
...
that legal) and the seven-part spelling still `cmp`s equal to `<mc/core>`.
...
* **Read**, on every `mc build`: `dep_line` / `dep_hash_tree`. A package installed with
  `files = ["evil.mc", "../../../payload.txt"]` hashed a file three directories above its own
  tree and the build came out `rc=0`.
* **Write**, `mc pkg vendor`: `pkg_copy_file` / `pkg_copy_tree` joined the same entry against
  `deps/<pack>/` and copied. With the project two directories deeper than the installed tree, the
  source resolved to `<libs>/../payload.txt` and the destination to a file OUTSIDE the project —
  measured: `/tmp/m44repro2/a/b/payload.txt`, content `PAYLOAD`, written by `mc pkg vendor`, exit 0.
* **Delete**, `mc pkg sync --yes`: on a hash mismatch `pkg_unbless` re-read the just-extracted,
  untrusted `mc.toml` and `unlink`ed everything it listed. A registry row with a deliberately wrong
  `sha256` and an archive whose `files` carried `../../../victim.txt` printed
  `mc: checksum mismatch for evil 1.0.0` — and `victim.txt` was gone. **The attacker never needs a
  hash that passes**: the wrong hash IS the trigger.
...
Against a URL registry it `return`ed — silently answering "immutable" — both when `--yes` was
...
## Implementation notes — step 4 (`mc install`, the slim binary)

The amendment's § B4/B5 as **M48 § 2.6 rewrote them**: `mc install` fetches the `mc` package's own
index row, and the name `mc-libs-<ver>.tar.gz` belongs to `mclib`. So there is no second archive,
no `--from-bundle`, no hand-written member list and no `src/bundle_empty.mc`.

**1. `mc install` is 174 lines and costs no global.** `src/install.mc`, in `<mc/core_pkg>` (not
`<mc/core_build>`, as § B4 wrote: since the tree comes from the registry it needs the index reader,
the fetch and the lock-era hash, all of which are `src/pkg.mc`'s). Everything it holds is a local
or already lives in `src/pkg.mc`'s record — the registry and `--yes` — so the seed's `globals` row
did not move for it.

Two roads and one layout:

```
mc install [VERSION] [--from-tree DIR] [--yes] [--force] [--registry URL|DIR] [--libs-dir DIR]
```

* the registry: `pkg_index_load("mc")` → `pkg_row("mc", VERSION)` → `pkg_plan` → `pkg_fetch_one`,
  which is download, `fetch_check_members`, extract, `dep_hash_tree`, compare, `pkg_read_meta`,
  manifest — unchanged, so a wrong `sha256` unblesses exactly what the extraction wrote and leaves
  no manifest;
* `--from-tree DIR`: the source is hashed, `pkg_copy_tree` copies `mc.toml` plus `[package].files`
  through the ONE reader that checks containment, the copy is hashed, and the two hashes must
  agree — two lines that make "the installed tree is the checkout" a checked claim rather than a
  copy that happened.

Then, on both roads, `tools/bundle.list` is copied to the ROOT of the tree: `dp_mc_load` reads it
there, the repository keeps it under `tools/`, and it is deliberately not a `[package].files` entry
because it must not move the hash the registry published (M48 § 2.6's note).

**2. `VERSION` is a positional, not a test-only override.** The task asked for a way to select a
row other than `mc_version()` so the registry road could be tested offline; an `MC_INSTALL_VERSION`
environment variable and a `--version` flag were both considered and dropped — the first is a
second way to configure the compiler, the second collides with the global `--version`. A positional
is a real feature (`mc upgrade`, step 5, has to fetch a version the running binary is not) and it
is documented as what it is: *the version to install, this compiler's by default, and the default
is the only one this binary's own `#include <name>` will read.*

**3. The sentinel refuses the registry road only.** `0.0.0-dev` is what the checked-in tree says,
and no registry publishes it: `mc: mc 0.0.0-dev is a development build: the registry publishes no
such version` + `run: mc install --from-tree .`, exit 2. `mc install 1.0.0` on a dev build is
allowed and does what it says — it installs a tree the running binary will not read, which is
exactly what step 5 needs.

**4. The refusal a slim binary owes its user cost one global and one function pointer.**
`unknown bundled include: prelude` is the wrong sentence for a compiler that bundles nothing.
`lex_set_libs` now takes two pointers (`lopen_fn`, `lhint_fn`); `src/lex.mc` asks the hint ONLY
when `bopen_fn == 0` — a binary with no bundle at all — and `src/deps.mc`'s `dep_include_hint`
answers 0 when the `mc` package IS installed, so the ordinary message is untouched everywhere else.
That is the same part-boundary shape `bopen_fn` and `lopen_fn` already are; the alternative
considered (a third `stage` on `libs_open` that returns a message instead of a source) would have
made one pointer mean two things to save one global. `globals` went 445 → 446 of the seed's 512.

```
prog.mc:1: #include <prelude>: not bundled in this compiler and mc 0.0.0-dev is not installed: run mc install
```

**5. The slim flavour is an assembly, not a part.** `src/core_slim.mc` (`<mc/core_slim>`) is
`core_min` + `core_machines` + `core_writers` + `core_build` + `core_pkg` + `main_slim.mc`;
`src/main_slim.mc` (`<mc/main_slim>`) is `src/main.mc` without `mc_bundle_init()` and
`mc_sandbox_init()`. Five entries — `src/mc_slim.mc` and one per cross target — and four
`src/mc.<target>-slim-obj.toml` configs, each a copy of its full sibling with `entry` changed.
`tests/pkg/nobundle.mc` was deleted: it existed only until this file did, and
`scripts/check-pkg.sh` § 15 now uses `src/mc_slim.mc` as its probe.

**Deviation, on record:** `<mc/core_sandbox>` is left out of the slim flavour as well as
`<mc/core_bundle>`. That is the part list the task named, and it is defensible (a Linux supervisor
is not part of compiling anything), but it makes the flavour two differences rather than one. Going
back to "slim is `mc` minus the blob" is one `#include` and one call in `src/main_slim.mc`, and
nothing else moves.

**6. Measured** (macOS arm64, `scripts/check-parts.sh`): the slim compiler is **534 179 bytes on
disk against 1 225 843** for the full one, 44%, with `__DATA,__data` **8 608 against 605 104**. The
bundled spelling and the checked-in entry produce the same object (659 400 bytes), which is
check-standalone's argument one flavour down. `_bundle_open`, `_bundle_read` and `_sb_go` are
absent from it; `_install_cmd`, `_pkg_cmd`, `_drv_build`, `_macho_write` and `_libs_open` are in.

**7. The release carries ten archives**, five full and five slim.
`scripts/release-assets.sh --slim` names the archive `mc-<ver>-<target>-slim.tar.gz` (§ B5's
spelling) and writes one extra paragraph of `INSTALL.txt`; the binary inside is still `mc` /
`mc.exe`, so `publish`'s `dist/mc-*.tar.gz` glob needed no change. The macOS job builds
`dist/mc-slim` and cross-compiles four slim objects into the two artifacts the full objects already
travel in; each Linux and Windows leg links its slim object beside the full one and packages it.
The slim binaries are **not** bootstrapped: the fixed point is a property of the compiler, and the
slim one is the full one minus a data section. `mc-libs-<ver>.tar.gz` is not produced here — it is
`mclib`'s name now (M48 § 2.6).

**8. The gate**, `scripts/check-pkg.sh` **111/111 → 129/129**, § 32, offline like everything above
it: `--from-tree` writes 100 files plus `mc.toml` plus `bundle.list` and the manifest carries the
hash `scripts/pkg-hash.sh` computes for this repository; a second run says `mc 0.0.0-dev is
installed` and does nothing; the sentinel refusal; `mc-slim` builds, reports its version, refuses
`<prelude>` with the exact sentence, INSTALLS ITS OWN PACKAGE, compiles that program to an object
byte for byte the full compiler's, and then compiles `<mc/host>` + `<mc/core>` + `<user_default>`
to an object byte for byte `src/mc.mc`'s — M44 § A3 step 3, through `mc install` this time; and the
registry road against an index row for the `mc` package whose archive is a tarball of this checkout
(plan without `--yes`, fetch with it, a wrong `sha256` refused with no claim left behind, an
unpublished version refused). `scripts/check-parts.sh` gained § 4a, the slim assembly.
The full binary's `unknown bundled include: nosuchlib` is asserted in the same section, so the hint
cannot leak into it.

**9. The pre-scan had to learn the second road, and that is where M48 § 2.6's open question got
its number.** `mc limits`'s byte-level pre-scan (`src/limits.mc`, `ps_bundled`) reached the bundle
through `bopen_fn` and nothing else, so a slim compiler estimated **32 nodes** for a project whose
sources are all `<...>`: every table grew from its seed (nodes 10 growths, ins 9, symbols 6) and
the heap peaked at 95 MB for a build the full compiler does in 44 MB. `ps_bundled` now falls back
to `lopen_fn` stage 1 — the installed `mc` package — with `virt = 0`, because that source came off
the filesystem and its relative includes are paths. Inert for a full binary: the bundle answers
first for every name it ships.

With it, `mc-slim build tests/pkg/std --limits` is **`ok`, grow 0 in every table**. The numbers
also answer M48 § 2.6's question directly: compiling `<mc/core>` costs **175 244 nodes and 104 MB**
out of an installed tree against **101 194 nodes and 44 MB** out of the blob, because the installed
`src/bundle_data.mc` is the checked-in mode-0 array (1.4 MB of `u64 x = { ... }`, one AST node per
element) while the copy a binary regenerates from its own blob is mode 1 (`#embed`, one `N_BLOB`
node). It costs memory and nothing else, the estimate covers it, and **step 4 does not rewrite the
file**: the rewrite needs a blob to emit from, and the binary that most needs it is exactly the one
that has none.

**10. What step 4 deliberately did NOT do**, so step 5 knows where the seam is: there is no
`mc upgrade` and no `LATEST` file, and `scripts/release-assets.sh` has no `--libs` (that name is
`mclib`'s asset, M48 § 2.6). `mc install` also does not read `[registry].url` from a project's
`mc.toml` — it is not run in a project — so a private registry is named with `--registry`.

## Implementation notes -- step 5 (`mc upgrade`)

Written after the fact, against the code that shipped. The owner's instruction of 2026-09-06
replaced three things in § D2 and they are the deviations below.

**1. The version is resolved out of the REGISTRY, not out of a `LATEST` file.** § D2 point 1 asked
for `https://github.com/minicompiler/mc/releases/latest/download/LATEST`, a one-line text file the
`publish` job would attach. It is not needed: step 3 already gave the compiler an index reader, MVS
and `pkg_pick`, and the `mc` package already has an index file with one row per published version.
`pkg_pick("mc", VERSION, -1, 0)` is the whole resolution -- it loads the index (printing the plan
and stopping when the index itself would have to be downloaded without `--yes`), validates an
explicit version, refuses a yanked one, and otherwise answers the newest non-yanked
non-pre-release row. A candidate is never chosen for you, exactly as in `mc pkg add`. So
`release.yml` gains nothing, there is no second thing to keep in step with a tag, and the
private-registry road works for `upgrade` for free.

**2. The asset url is DERIVED from the index row (decision, option (a) of the task).** The registry
is a registry of sources, so a row carries the tag archive. One rule, for one forge, written down
in `src/upgrade.mc`:

```
https://github.com/<owner>/<repo>/archive/refs/tags/<tag>.tar.gz
  ->  https://github.com/<owner>/<repo>/releases/download/<tag>/mc-<ver>-<target>[-slim].tar.gz
```

with the checksum at that name plus `.sha256`, which is what `scripts/release-assets.sh` writes.
Anything else is `mc: upgrade: no binaries known for: <url>`, exit 2 -- refused rather than
guessed at, because a forge that is not GitHub publishes its assets somewhere else and a guess
would download the wrong file. Rejected alternatives: a `[registry].binaries` base (a second thing
to configure, and wrong the moment two packages in one registry live on two forges) and a new row
key (the registry's schema would have to grow a column only one package ever uses).

**A row whose url is a LOCAL PATH is the second road, and it is one line**: the assets sit beside
the archive. That is `--from DIR` from § D2 without the flag -- `--registry DIR` over a directory
holding a release already means it -- it is M44 § Risks 18's air-gapped upgrade, and it is what
makes `scripts/check-pkg.sh` § 33 offline.

**3. The swap is write-`.new` + `chmod` + `rename`, not `unlink` + write.** § D2 point 5 chose the
second; the owner's instruction chose the first, and it is stronger for the same reason: a rename
is atomic, so there is no instant at which the compiler on someone's `PATH` is half a file, and it
still gives the destination a NEW INODE, which is what the macOS cached-signature `Killed: 9`
needs (CLAUDE.md § M12). `upg_write_exec` reads the extracted binary and writes `<dest>.new` with
`creat(..., MODE_755)` after unlinking it -- a copy and not a rename of the extracted file,
because `<libs>` and `/usr/local/bin` are routinely different filesystems.

**Windows keeps § D2's shape but not its mechanism.** `MoveFileExA(self, self + ".old", ...)`
would have needed the running process to survive its own image being renamed away, and there is no
Windows runner in this repository that executes `mc upgrade`. What ships instead: the new compiler
is written to `<dest>.new` and the `move /y` command is printed. `rename` itself IS implemented on
that host -- `lib/sys_windows_host.mc` gained it over `MoveFileExA` with
`MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED`, plus `MoveFileExA` and `GetModuleFileNameA` in
`scripts/sysroot-windows.sh`'s `kernel32.def` -- so `mc upgrade --to PATH` works there like
anywhere else, and only the running binary is the special case. **This half is proved by reading**:
the Windows CI legs cross-compile and link, they do not run `mc upgrade`.

**4. `host_self_path()` is the one host answer this step needed.** `_NSGetExecutablePath` on macOS,
`readlink("/proc/self/exe")` on Linux, `GetModuleFileNameA(0, ...)` on Windows -- never `argv[0]`,
which is whatever the caller passed and after a `PATH` lookup is a bare name. It costs no global on
any host: the buffer is `xalloc`ed, not a `u8 buf[]` at file scope (which would have moved
`check-limits`'s `globals` row).

**5. The dev sentinel refuses the road that CHOOSES, and only that one.** A bare `mc upgrade` on
`0.0.0-dev` is `build from the tree` + `run: make mc1`, exit 2; `mc upgrade VERSION` is allowed and
does what it says. That is § D2 point 2's own rule ("a dev build refuses unless `--yes VERSION`
names one, so `make check` never clobbers `build/mc-exe`, and the acceptance test can") and
`src/install.mc`'s precedent for the same string. It is also what makes § 33 cost ONE extra compile
instead of two: the running client is `build/mc1` with a version named, and the only versioned
binary the gate builds is the one inside the fixture release.

**6. There is no `--force`.** The task's ruling: a downgrade is not a mistake to override, it is a
`VERSION` named explicitly, so the plan says `downgrade mc 9.9.9 -> 1.0.0` where it would otherwise
say `upgrade`. Nothing is refused.

**7. Two checks after the download, and they answer different questions.** The `.sha256` is
verified BEFORE `tar` is spawned -- the one thing this road can do that `mc pkg`'s cannot, because
a release asset has stable bytes and a tag archive does not (§ 3). Then the extracted compiler is
RUN once (`mc --version`, through `fetch_spawn_to`, the file-action trick that is the only way this
language captures output) and must answer the version that was asked for: that catches an asset
built from the wrong tag, which no checksum can. Neither is provenance, and
`docs/reference/packages.md` § 11 now says so in the paragraph § D3 asked for.

**8. The libraries are installed by SPAWNING the new binary**, unless `--no-install`. The tree is
`<libs>/mc/v<version>/` and the version that names it is the one the compiler REPORTS, so the
process that must ask is the one just written. Its exit status is the command's. With `--to PATH`
the spawned binary is the one at `PATH`, which is what makes the gate able to test the install road
without replacing anything.

**9. Cost.** `src/upgrade.mc` 407 lines (131 comment, 23 blank, **253 code**), `src/host_macos.mc`
+18/8, `src/host_linux.mc` +16/8, `src/host_windows.mc` +16/8, `src/core_pkg.mc` +9/3 --
**466 added lines in `src/`, 280 of them neither comment nor blank**, against the table's ~250 for
the file. The excess over the estimate is the two roads of the asset rule, the version check after
the extraction, the Windows branch of the swap and this repository's comment density; the plan
alone is 15 `out_str` lines, one per fragment, which is how every other message here is written.
**Zero new globals**: `check-limits` reports `globals 446/512` before and after. Outside `src/`:
`lib/sys_windows_host.mc` +22/-4 (8 code), `scripts/sysroot-windows.sh` +2, `tools/bundle.list` +1,
`mc.toml` +1 (`src/upgrade.mc` in `[package].files`, byte order), `scripts/check-pkg.sh` +231.

**10. The gate is `scripts/check-pkg.sh` § 33, and it is offline like everything above it.** The
fixture is real: a compiler built from THIS tree with one line of `src/version.mc` changed to
`9.9.9` (`scripts/set-version.sh`'s edit, applied to a copy so the checkout keeps its sentinel),
packaged by `scripts/release-assets.sh` into a directory that also holds the source tarball the
index rows point at. That is the only way to have a binary that answers `mc 9.9.9`, and it is what
makes the version check, the `cmp` against the packaged bytes and the self-replacement mean
something. Four rows -- `1.0.0`, `9.9.9`, `10.0.0-rc1`, `11.0.0 yanked` -- so that "the newest" has
something to skip. Twelve assertions: the dev refusal, the plan compared byte for byte, the GitHub
derivation and the other-forge refusal, a tampered archive (nothing written, no download left
behind), an asset built from the wrong tag, the happy path (`cmp` against the packaged binary,
`codesign --verify` on macOS, an empty download directory), the install road (`v9.9.9.toml` and
`bundle.list` at the root of the tree), `is the newest`, the downgrade plan, a yanked version named
by hand, and the self-replacement with no `--to` at all -- inode before and after, and the
replaced file reporting `mc 9.9.9`.

**11. What step 5 deliberately did NOT do.** There is still no `LATEST` file and no `--from`
(note 2 explains why neither is needed), no signing key (§ D3 stands, priced and unbuilt), and no
`mc upgrade` on the Windows runners. `release.yml` is untouched: the assets `mc upgrade` reads are
the ones it already publishes.

## Implementation notes -- version constraints (roadmap 1.0.0)

**The `[deps]`/`[tools]`/`[package].mc` version-expression syntax**, a FORMAT item that freezes
with semver at 1.0.0. A value is a constraint, not just a minimum: bare `X.Y.Z` and `>=X.Y.Z`
stay a MINIMUM (unchanged), and `=X.Y.Z`, `~X.Y.Z` (patch), `^X.Y.Z` (minor, with the SemVer/npm
0.x narrowing) and `*` (any) add an upper bound. `>`/`<`/`<=` are deliberately absent: MVS's model
is a lower bound plus a caller-chosen ceiling, so a ceiling is always derived from `~`/`^`/`=`.

* **One parser**, `ver_parse(cs, out)` in `src/deps.mc` beside `ver_cmp`, into a VC record
  (`VC_MIN`/`VC_HIGH`/`VC_EXACT`, no allocation), reusing `ver_field`/`dep_semver_ok`/`ver_cmp`;
  `ver_satisfies(out, v)` is the half-open test. Every reader goes through it: `[deps]`/`[tools]`
  (`pkg_read_deps` on the sync side, `deps_apply` step 3 on the build side), a registry row's own
  dependency version (`pkg_expand`, so a row may publish `"mathx ^1.1.0"`), and `[package].mc`
  (`dep_enforce_mc`, which now describes a capped constraint's range in its message).
* **MVS with ceilings** (`src/pkg.mc`): the selection record gained `SL_MIN`/`SL_HIGH`/`SL_EXACT`,
  accumulated across every requirement (`SL_MIN` = max of mins, `SL_HIGH` = min of ceilings). The
  concrete `SL_VER` is recomputed by `pkg_reselect`: for a name constrained only by bare/`>=`
  versions it is `SL_MIN` with NO registry lookup -- byte for byte the M44 maximum-of-minimums --
  and for a ranged/exact/`*` constraint it is the LOWEST registered version in `[low, high)`
  (`pkg_lowest`). An empty intersection, no version in range, an exact outside the range, or two
  disagreeing `=` pins is a conflict, exit 2, naming the package and the bounds; a cross-major
  clash stays the different-majors error. The lock stays CONCRETE.
* **`mc update`** raises a `~`/`^` anchor to the newest version within the constraint's ceiling
  (`pkg_highest`) and keeps the operator, leaves `=`/`*` untouched, and for a bare/`>=` behaves
  exactly as M44 did (newest of the same major).
* **Backward compatibility, proven**: `scripts/check-inert.sh` against a pre-change compiler is
  33 objects and five taught examples byte-identical on both roads; every existing `tests/pkg/*`
  fixture, `mc.lock`, `mc.lock.expect` and `tests/golden/pkg-list.txt` is unchanged (the bare/`>=`
  path is untouched). New fixtures `tests/pkg/src/zero-0.*` (a 0.x package) and `zdep-1.0.0` (a
  registry caret), `scripts/check-pkg.sh` § 34 (every operator, an intersection, two conflicts,
  four malformed cases and three `mc update` cases). Docs: `docs/reference/toml.md`,
  `docs/reference/packages.md` (§ MVS, § the minimum mc version, § Refusals),
  `docs/reference/diagnostics.md`.

### The stale index (post-step-5 fix)

Reported by the teko consumer against mc 0.15.23 with a pure-mc reproducer, and confirmed by
reading `src/pkg.mc`: **the snapshot of a URL registry was never refreshed.** `pkg_index_file`
answered `if (lex_readable(snap)) return snap;` before it looked at `--yes`, so the index was
fetched on the first query of a package and read from the disk for ever after -- a version
published later was invisible, `mc pkg sync --yes` said `mc: teko 0.10.0: no such version in the
registry` with `https://pkg.minicompiler.dev/index/teko.toml` already listing it, `--registry URL`
did not help, and `rm ~/.mc/libs/index/teko.toml` was the only way out.

**The rule implemented is § 5's own promise read the other way round.** The snapshot exists so that
a project's LATER `mc pkg` calls work offline and so that `mc build` never touches the network --
and `mc build` reads `mc.lock`, never the index, so the offline promise is not the snapshot's to
keep. `--yes` means "you may download", and the roads that carry it (`sync`, `add`, `mc update`,
`mc install`, `mc upgrade`) are exactly the ones asking the registry a question whose answer
changes over time. So with `--yes` the snapshot is REFRESHED where it used to be read, at most once
per package per invocation because `pkg_index_load` memoises what it has read; without `--yes` it
is read exactly as it is.

Refresh-on-miss-and-retry was considered and rejected as insufficient, not merely as bigger: a
stale snapshot answers `pkg_newest`, `pkg_lowest` and `pkg_highest` with a version that EXISTS, so
there is no lookup miss to trigger a retry and `mc pkg add NAME` / `mc update` would silently
choose the old one. Refreshing at the one funnel is both correct and the smaller diff.

The second half is the negative answers a snapshot a command may not refresh should not give:
`pkg_index_maybe_stale(name)` (a URL registry and no `--yes`) guards the three of them --
`pkg_expand`'s missing row, `pkg_pick`'s missing or nothing-to-choose-from, and `pkg_reselect`'s
empty range -- and each plans the index fetch instead, which lands in the existing
`nothing was downloaded: re-run with --yes` at exit 0. `pkg_index_load` already returned 0 under
exactly that condition when there was no snapshot at all, which is why the two cases merge into one
branch at each site. No message was added and none changed. `pkg_check_immutable` was left alone: it
fetches the published index into its own `<snapshot>.published` and must treat a 404 as "a new
package" rather than dying, so it is a different read and not a duplicate of this one.

Cost: `src/pkg.mc` +71/-20, **33 added lines that are neither comment nor blank**, zero new globals
(`mc limits src/mc.mc` reports `globals 446/512` before and after). `scripts/check-pkg.sh` § 36
(177 -> 183) is the gate, and it is offline like everything above it: a THIRD bin directory whose
`curl` serves `$WEBROOT` instead of exiting 97, so the transfer is a file copy through exactly the
argv `fetch_get` builds, with the two refusing downloaders still on PATH for every other section.
Six cases -- the first sync writing the snapshot, the reproducer itself (a row published after it,
found with nothing deleted by hand), a version that truly does not exist still refused with the
same message after one refresh, the no-`--yes` plan with the snapshot left untouched, `mc pkg add`
picking the newly published newest (the road a miss-and-retry rule could not fix), and `mc build`
green on a locked project under the REFUSING downloader.

### The vendored tree at another version (post-step-5 fix)

Reported by the teko consumer with a pure reproducer, and confirmed by reading `src/pkg.mc` and
`src/deps.mc`: **a checked-in `deps/<pack>/` was never asked which version it is.** `deps/` wins
over the cache (D10', § 3), and `pkg_present`/`pkg_tree_dir` decided that on one question --
"is there an `mc.toml` under `deps/<pack>/`?" -- so a project whose `[deps]` had moved to version A
while the checkout was still at B did not fetch A at all: it hashed B and compared that hash with
A's `sha256`.

```
mc: checksum mismatch for zero 0.2.0
  expected 1177e8ee4bfcf6578b269554cd133230c9402a3fd3d76a40a25c71ea6235809b
  got      9943772d3362833b3749d496d9d43c7303c22faa6ccdfbc7492ea348903f05d7
```

The `got` line is `mc pkg hash deps/zero` exactly -- the consumer checked. The diagnosis is wrong
twice over: nothing is corrupt, and the advice a checksum mismatch carries (re-fetch, re-sync, look
for a tampered byte) does not apply. Moving `deps/zero` aside made the same command fetch and lock
without complaint, which is what turned it into a report.

**`mc build` said the same thing in its own words**, and the check confirmed it is the same defect:
`dep_resolve` picked the vendored tree on the same one question, and `dep_scan` then produced
`mc: zero 0.2.0: the tree does not match mc.lock` with `run: mc pkg verify` -- and `mc pkg verify`
repeats it, so the advice is a loop.

The rule is **the version before the hash**, in one function called from all three places:
`dep_vendored_at(name, dir, ver, from)` (`src/deps.mc`) answers "is `deps/<name>/` this package at
this version?" and is the sole reader of the vendored tree's own `[package].version` -- a new,
OPTIONAL key. 0 is "no tree there", 1 is "a tree that declares nothing, or declares this version",
and a tree that declares another one never comes back:

```
mc: deps/zero is 0.1.0, [deps] wants 0.2.0: update the checkout or remove deps/zero
```

`from` is where the requirement was read, which is the only part that differs between the roads:
...
Nothing is fetched and no lock is written; `mc pkg vendor` refuses too, and the answer there is the
one the message already gives -- remove the checkout, then vendor.

**A tree that declares no version behaves exactly as before**, which is what makes the change inert
for every package published so far (none carries the key) and for every fixture but the two the
gate uses. That is also the honest limit: without a declaration there is nothing to compare, and a
mismatched checkout can still only be reported as a hash. The key is documented as being read in
this one place and as resolving nothing -- the registry picks a version, `mc.lock` pins it, and a
fetched or installed tree carries its version in the manifest beside it. A malformed value is
`deps/<pack>/mc.toml:L:C: not a usable version`, exit 2, at its own position, like `[package].mc`.

Cost: `src/deps.mc` +39/-5 and `src/pkg.mc` +11/-4, **24 added lines that are neither comment nor
blank**, zero new globals (`mc limits src/mc.mc` reports `globals 446/512` before and after).
`scripts/check-pkg.sh` § 39 is the gate (194 -> 200 checks): the reproducer itself, the checkout
updated to the resolved version (vendored still wins, still nothing fetched), `mc build` and
`mc pkg verify` naming the version instead of the bytes, a tree with no declaration still taking
the hash road, and a malformed declaration. `tests/pkg/src/zero-0.1.0` and `zero-0.2.0` gained the
key; no committed hash names them, so no lock and no golden moved for it.
