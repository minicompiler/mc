...
| **M39** | An architecture taught from the surface (owner, 2026-09-04): `examples/kernel`, a bare-metal RISC-V 64 micro-kernel (reset, NS16550A UART, `mtvec` trap, two cooperative tasks, `ok`, SiFive test-device exit) compiled by a taught compiler that registers a RISC-V machine, a flat-image writer and a bare-metal layer, all in `.mc` under `examples/` -- zero lines in `src/`; run under `qemu-system-riscv64 -machine virt -bios none` as the CI oracle; the gaps this exposes in the core are priced as follow-ups (M39.5: `mc build` with a module-registered `[target]`; M40: the word-size sweep AVR/PIC need) (`docs/specs/M39.md`) | `git diff --stat src/ stage0/ lib/ tests/` empty; the image boots and exits 0 in QEMU with the exact transcript; `llvm-mc` re-assembles every distinct instruction byte for byte; golden unchanged |
| **M39.5** | `mc build` with a module-registered `[target]` (gap G1 of `docs/specs/M39.md`, decision D2, done 2026-09-04): the deferral form only -- `drv_run` keeps `[target].os`/`.arch` as strings, `drv_entry` passes a role (obj/exe), and `drv_backend_for` resolves the pair inside `drv_parse` right after `user_init()` and before `parse_unit()`; the two registry-built diagnostics and the `requires [linker]` check move unchanged, `drv_teach`'s host lookup is untouched, and there is never a second user entry point. `examples/kernel` is the consumer: `[target] os = "none" / arch = "riscv64"`, registered by its own compiler with `rv-image` in both roles, so `mc build examples/kernel` writes the image end to end | 18 added / 15 removed code lines in `src/driver.mc`; the image byte-identical to the one the single-file CLI wrote; `check-build` 16/16 with its `[target]` messages unchanged; a pre-change `mc1` writes identical objects for every `tests/*.mc`, for `src/mc.mc` and through all four taught compilers |
| **M40** | The narrow word (owner, 2026-09-04): `examples/avr`, a bare-metal ATmega328P image (blink, console `ok`, one timer ISR, exit through simavr) built by a taught compiler under design A -- the word stays 8 bytes and the AVR machine widens, zero lines in `src/`; ELF32 `EM_AVR` written by the module with the `.mmcu` section; simavr primary and `qemu-system-avr` second as oracles; narrow `u8`/`u16` arithmetic via M24's depth type, declared; PIC excluded (banked memory and a hardware call stack are a different frame model); G10 repriced from ~400 lines to ~15 plus one semantic decision, superseded the same day by the owner's override direction: the AVR module declares `uptr = 2` from the surface and the recreated compiler is debloated (`docs/specs/M40.md` § Amendment; after M24 and M41) | `git diff --stat src/ stage0/ lib/ tests/` empty; the image runs under both oracles with the exact transcript; the ELF and the vector table match an `avr-gcc` reference; `llvm-mc -triple=avr` re-assembles every distinct instruction |
| **M41** | Override, debloat and re-arch (owner, 2026-09-04): the compiler as a product of the developer's own build -- `<mc/core>` composable (a minimal core of lexer, parser, resolver, walker and hooks; writers, host machines, driver, bundle and sysroot resolver as optional parts), primitives removable (types and intrinsics unregistered; writers, machines, targets simply not registered), and the core's remaining fixed decisions reachable as overrides from `user_init()` (the word width of `uptr` first); `examples/avr` is the first recreated compiler (`docs/specs/M41.md`; after M24). **Done 2026-09-04** in four gated commits: five parts (`<mc/core_min>`, `<mc/core_machines>`, `<mc/core_writers>`, `<mc/core_build>`, `<mc/core_bundle>`) with `src/core.mc` as their sum, `mc_main()` in `src/cli.mc`, the `subcommand`/`on_plan`/`backend_default`/`machine_use_if` registrations, `type_disable`/`intrinsic_disable`/`type_set_width`, and `scripts/check-parts.sh` as the gate; a core_min-only compiler is 219 417 bytes against `mc`'s 759 875, 29% | a taught compiler built from the minimal core is smaller than `mc` by the parts it omits, measured; a program the stock compiler accepts is refused by a debloated one that removed the primitive; objects identical for every compiler that overrides nothing |

M42 onward (Linux `--exe`, Windows PE `--exe`, `i32`, the package manager, the sandbox, the
package-permission work numbered M48) landed after this table stopped being kept in step with
`main`; they are recorded milestone by milestone in `CLAUDE.md` § State and in their own
`docs/specs/Mxx.md`, not here. The three rows below are new, added from the 2026-09-06 benchmark
(`docs/comparison.md`) rather than from an owner directive, and use the next free milestone
numbers as of that date.

| # | Deliverable | Acceptance |
|---|---|---|
| **M49** | An optimizer (from `docs/comparison.md`, measured 2026-09-06: `mc` at `clang -O0` level, 2.2x behind `clang -O2`, on a workload of an LCG/xorshift mix, a sieve and recursive `fib`): a register allocator for locals and call-live values over the callee-saved registers (the walker already tracks per-depth liveness through its depth stack), a peephole over the `Ins` buffer (store-then-load, redundant moves), constant propagation and small-leaf-function inlining as `pass()`-level AST rewrites ahead of `gen_walk`. **Estimate, not a promise**: within 1.3x of `clang -O2` on the same workload | the workload benchmark re-run shows the ratio; `check-inert` holds for every program that already folds to the same instructions; `--dump-asm` diff empty for every corpus program an allocator cannot improve (nothing live across a call) |
| **M50** | A reproducible bench cell (from `docs/comparison.md`): `bench/Dockerfile` with every compared toolchain pinned to an exact version, run one container per row on a fixed VPS with `--cpus 1 --memory 512m`, results committed as dated JSON so a regression is a `git blame`, not a chat log; replaces the two honest caveats `docs/comparison.md` § Conditions records today (shared host load, an unverified tree label). Option, not a decision: the soak (`.github/workflows/bench-soak.yml`, `docs/comparison.md` § "The hour under load") already runs one pinned-toolchain job per server on GitHub-hosted runners, so this cell could be a GitHub Actions job per row instead of Docker on the VPS (`docs/specs/M50.md`, which measures both roads and recommends Actions: one job per TOOLCHAIN with a `clang -O2` reference timed beside every row, because the same reference binary has swung 33% between sessions on one host while the ratio holds to 0.7% back to back). **Done 2026-09-13** in three gated steps, on Actions and not on the VPS (which is production and is itself a shared VM; Docker is kept priced as an option for a road off GitHub): `bench/cell/cell.py` + the phase argument in all six sources (A), `.github/workflows/bench-cell.yml` over `macos-15` + `ubuntu-24.04-arm` + `ubuntu-latest`, 18 jobs in 17.2 min of runner time (B), and `bench/cell/gate.py` + the first committed `bench/results/<date>-<run>/` (C). It gave M49 step D2's x86-64 allocator its first timing -- **19-21% on `mix`** against 130-265% on AArch64 -- and `docs/comparison.md` § Conditions lost both caveats for a cell-of-record line naming the run, the verified asset checksum and the timed binary's digest | two consecutive runs of the cell agree within a documented tolerance; a deliberately regressed `mc` build fails the cell -- **both met, and the first refuted the spec's own recommendation**: the teeth are the ratio a cross-run comparison can carry (0.13-5.1% drift, and a `--opt=0` build fails on all three cells with `tooth 1.001/0.981/1.006 below 1.50/1.50/1.10`), while a row's ratio to a per-job reference agrees within 5% on `linux-arm64` alone and is therefore reported, not gated |
| **M51** | A bundled `<http>` library (from `docs/comparison.md`): `#include <http>` for the fork-per-connection-with-keep-alive shape measured as `mc-forkka` in `bench/http/` (120k req/s, 12.5 CPU ms/1k on this host, second-lowest CPU cost measured after Rust/Zig), with an event-loop shape to follow once one exists; the fork-per-**request** shape (`mc-fork1`, 335 CPU ms/1k) is what a program should no longer have to hand-roll. **The registry server's own migration onto this library is `minicompiler/mc-registry`'s work, not this repository's** -- this milestone only ships the library. **Superseded on one point by `docs/specs/M52.md` § 9.3 (D13): `http` ships as a registry PACKAGE, never as a bundle row** -- M52's rule puts every `lib/` path outside the blob, HTTP is not a dependency of the compiler, and its release cadence is not the compiler's, so M51 becomes the first per-library package of M52 § 9.2 and inherits its two costs (`bench/http/`'s single-file programs become a project; the name `http` wants registering before the first tag) | `bench/http/mc-forkka`'s shape becomes `#include <http>` plus a handler function; the suite's own examples that speak HTTP (`examples/api`) can move onto it without changing their route table |
| **M52** | The standard library becomes installable, and the binary carries only the compiler's own source (`docs/specs/M52.md`, from the owner's direction of 2026-09-08: "os bundles podemos converter em libs que podem ser instaladas (caso não existam) no momento do build de uma aplicação. Dessa forma o binário do mc fica mais enxuto, portando somente o que é estritamente essencial e verdadeiramente dependência"). One rule applied per bundle row -- a row stays iff its path is under `src/`, plus `prelude` and `user_default`, the two files sixteen `src/` files and `src/user.mc` include -- takes **41 of 101 rows and 122 838 of 678 368 blob bytes** out: the float library and its two machines, the wide integers, half precision, the system layers, stdio, seventeen check fixtures (the largest single row in the whole blob is `user_syntax_demo`, 21 005 B) and the two `#embed` payloads. **The mechanism needed no design**: resolution step 3, `<libs>/mc/v<version>/` on a bundle miss (`docs/reference/packages.md` § 2), already serves them, and the spec proves it end to end on the single-file CLI and through a taught compiler whose object is `cmp`-identical either way -- so the libraries stay files of the `mc` package (41 of its 105 `files`), **no `#include` spelling changes anywhere**, and no new registry package is published. The compiler-side work is one predicate in `tools/bundle.mc`, two more roots in `dp_mc_load` (`<exe>/lib/mc/v<ver>/` and `<exe>/../lib/mc/v<ver>/`, so a release ships the tree beside the binary and "untar and a float program compiles, offline" survives), one widened refusal, and `mc build --sync` -- about **55 lines of `src/`**, `stage0/` untouched. `mc build` still never downloads: an implicit newest-resolution is refused on determinism grounds. Publishing `float`, `i128`, `io` as packages of their own stays priced and deferred (§ 9.2, on top of `docs/specs/M47-S5.md` § 4) for one measured reason -- only ONE bare name per package survives, so 35 of the 41 names would have to be respelled in every consumer -- and `mclib` (`docs/specs/M48.md` § 2) is retired rather than implemented. Paired with teko as the **0.16.0** minor: the CUT is coordinated, the design is not, and teko needs **no `[deps]` line and no respelling**. **Done 2026-09-14** in three gated steps: **A** the library root beside the binary and the refusal that names `mc install` (`dp_mc_root()`, three roots -- `<libs>`, then `lib/mc/v<ver>/` beside the executable, then one directory up for a packager who puts `mc` in `bin/` -- 86 added lines, 37 code); **B** the cut (`bl_in_blob`/`bl_cut` in `tools/bundle.mc`, **101 rows -> 60**, 41 to the tree; **the binary 1 400 552 -> 1 268 454 B, −9.4%, `__DATA,__data` −18.0%, `__text` and `__cstring` byte-identical**; 52 code lines in `src/`, none of them the predicate -- a `[compiler]` child and the sandbox box are both blind to a beside-the-binary root and had to be told); and **C** `mc build --sync [--yes]`, the one road from a build to the network -- `pkg_sync` behind a function pointer `mc_pkg_init()` registers, so `<mc/core_build>` still stands without `<mc/core_pkg>` (**79 added lines in `src/`, 30 of them neither comment nor blank**, against the spec's ~20; zero new globals). `mc build` without the flag is byte for byte the command it was, and `check-pkg`'s `curl`/`wget`/`tar` shim keeps proving it | measured, not projected: the binary 1 408 576 -> **1 268 276 B** (−10.0%), `__DATA,__data` 691 456 -> 566 832 (−18.0%), and **`__text` exactly 540 192, unchanged -- a bundle row is data**; `mc-slim` 626 420 B is the floor and stays a flavour; `check-obj` 32/32, `check-inert` identical on both roads, `check-standalone` still `cmp`-identical to `src/mc.mc`, `check-float`/`check-wide`/`check-surface` unchanged in outcome, a new `check-libroot` for the three roots and the refusal, `make check` green with no network, ten goldens rewritten once per step |
| **M53** | The surface freeze, the deprecation policy and the release canary — the road from 0.16.0 to RC 0.17.0 and to 1.0.0 (`docs/specs/M53.md`, from the owner's direction: "Congelar a superfície — RC 0.17.0 — crítico. Política de deprecação (hooks.md § Stability): de 1.0.0 sem rename seco — adiciona novo + alias por um major, remoção = MAJOR. Formatos, mc.toml/mc.lock, nomes `<mc/*>` e a versão congelados = semver de verdade" and "Melhorias + canário — antes de cortar: a release do mc dispara o CI da teko antes de publicar (bloqueante no 1.0.0). Sem credencial — a teko puxa a release pública do mc"). **The inventory already exists and nobody reads it as one**: `scripts/check-docs.sh` extracts the whole public surface from `src/` on every `make check` — 206 symbols, 50 CLI flags, 35 TOML keys, 10 directives — and throws it away after asking "is each one documented?". M53 asks the second question, "was each one here last time?", for **seven kinds and 420 entries**: those four plus the 101 `tools/bundle.list` names (the `<mc/*>` parts and the libraries, one kind — M52 moved 41 of them out of the blob without changing a name), the 14 `mc.lock`/index row keys (a computed TOML path, so `check-docs`'s regex never saw the format at all), and one line carrying the machine contract's version. **A hole found and fixed before the freeze, not after**: `val_reg`, `dst_reg` and `dst_done` are published BY NAME in `docs/reference/machine.md` § 3 as the only three names of a machine's internals that are frozen, and they match none of the 17 prefixes — 0 of 206 — so the symbol count becomes **209**. Deliberately NOT frozen, each with its own argument: `src/`'s **448 file-level globals** (freezing them would have made M48 C0's globals diet a MAJOR, and that diet is what keeps `check-limits` under the frozen seed's ceiling — the migration is an accessor per global a consumer needs, which is then frozen, and a global rename goes in the release note); and diagnostic TEXT (373 documented messages against 19 a gate asserts — the exit code and the stream are the contract, a wording is a PATCH). The mechanism is one shared extractor both gates read (`scripts/surface-extract.sh` — two regexes that must agree are a bug waiting), one recorded golden (`tests/golden/surface.txt`, re-recorded with `--record` and not by deletion, because unlike a hash its content is the review artefact), and `check-freeze` inside `make check`: a removal fails naming the MAJOR rule, an addition fails until re-recorded, so every surface change costs one committed line in the same pull request. The canary needs **no credential in either direction**: every release is born a GitHub pre-release, teko polls the public releases API, runs its recipe and writes `canary/<version>.json` to a branch of its own repository with its own token, and a `promote` job on this side polls that raw URL for 90 minutes and flips `prerelease: false` — advisory before 1.0.0, required at it, with `publish-to-registry` moved behind it so a candidate never becomes the registry's newest row. **0.17.0 is an ordinary minor and "RC" is a state of the repository, not a suffix**: two places refuse a `-` suffix on purpose, and a pre-release version would be skipped by `mc upgrade` and `mc pkg` — a candidate nobody can install is not one. `src/` and `stage0/` untouched, **0 lines of `src/`, and not one of the ten goldens moves**. **Steps A, B and C landed 2026-09-14**: A is the extractor and `check-freeze` (`tests/golden/surface.txt`, 420 entries, `git diff --stat src/ stage0/` empty); B is `docs/reference/hooks.md` § 8 written in full, the six one-line pointers from `cli.md`, `toml.md`, `packages.md`, `bundle.md`, `objects.md` and `machine.md`, and `docs/ci.md`'s label table gaining a "what the policy allows" column plus the RC-is-a-state line — docs only, no golden moves, `check-docs` green at 570 relative links; C is the canary workflow itself (`release.yml`'s `MC_CANARY` pre-release flag, the `promote` job, `scripts/canary-poll.sh`). Step D, the compile-time deprecation note, stays deferred to the first real deprecation (D13). **The milestone does not close on A-C**: it closes when teko's own gap list reaches zero and it self-hosts on its five legs (§ 13) — A, B and C ship in 0.17.0 on their own schedule and do not wait for that | the inventory is 420 entries (209/50/35/10/101/14/1) and `tests/golden/surface.txt` is `surface-extract.sh`'s output byte for byte; `check-docs` green at 209 symbols with no doc edit; the gate has teeth in four directions, each proved in the commit — a rename fails, an addition fails until re-recorded, a deprecated-then-removed entry fails until re-recorded, and the machine version moving down or up without a changelog paragraph fails; `git diff --stat src/ stage0/` empty, `check-inert` identical on both roads, `check-obj` 32/32, ten goldens unmoved; the canary round trip measured once end to end in advisory mode against a real teko run, under the 90-minute timeout, and a `fail` verdict leaving the release a pre-release with every asset attached; `make check` green with `check-freeze` in it and `make check-linux-host` RC 0 over all four cells |
...
**What is promised, from 1.0.0 onward:**

* **The recorded public surface does not shrink without a MAJOR.** Seven kinds, extracted from
...
* **The ten committed fixed points keep holding, on all five hosts `mc` runs itself on** — macOS
  arm64, Linux arm64, Linux x86-64, Windows arm64, Windows x86-64 — on the plain road and on
...
* **A `src/` file-level global.** 448 of them; a consumer reads a compiler internal through a
  *function*, and the function is what freezes (`val_reg`/`dst_reg`/`dst_done` are the three the
  machine contract already publishes by name). A rename of a global nothing wraps goes in the
  release note, not through the gate.
* **Diagnostic message TEXT.** `mc` documents hundreds of messages; the exit code and whether the
  text went to stderr are the contract. A wording is a PATCH; an exit code is a MAJOR.
* **An emitted byte, an object layout beyond its own format's standard, or a golden hash.** That
  is `docs/determinism.md`'s promise (reproducible, not frozen) — it moves by design whenever the
  compiler improves, which is what the M49 optimizer already proved.

**How a consumer pins to a version:** `[package].mc` in its own `mc.toml` — a minimum-version
constraint on the compiler that may build it (`=`, `~`, `^`, `*`, `>=`); `mc upgrade`, which
downloads a specific release, verifies it against the checksum published beside it and replaces
the running binary only after the new one confirms its own version by running; and, for the
registry's own gate, the pinned validator version it raises at every cut so a manifest naming
`[package].mc` or a semver constraint is never refused by a validator older than the syntax it is
reading (`docs/specs/M52.md` § 16).
