...
| `release.yml` | dispatched by `autotag.yml`/`tag.yml`, tag `v*`, or manual | `macos-15` + `ubuntu-24.04-arm` + `ubuntu-latest` | builds `mc` for macOS, cross-compiles the two Linux objects, links and bootstraps each on its own architecture, packages all three, publishes the GitHub Release and announces the tag to the mc package registry |
...
it is a property a later job could change. So a release candidate is a *state of a release*, never
a suffix; `mc upgrade` and `mc pkg add mc` skip a
...
the GitHub release's pre-release flag instead.
...
`-` suffix (`0.2.0-rc1`) is published as a pre-release, and nothing else is. This job is the only
one with `contents: write`.
...
The action's own defaults are the registry: the apex (`https://minicompiler.dev`)
for `POST /poll` and `https://pkg.minicompiler.dev` for the index. Between
v0.15.4 and the apex flip of 2026-09-06 the job named `next.minicompiler.dev`
explicitly, because the apex was still GitHub Pages and answered **405** to
`POST /poll` (measured on v0.15.4's run); those two inputs were deleted at the
flip and nothing else in the job changed.
...
It waits on `publish` alone. M53 step C put a release canary between the two —
every release born a pre-release, promoted only on a consumer's verdict — and
that mechanism was **removed on 2026-09-23**, because it existed for a
co-dependency with teko that no longer exists. A release is published, and
announced, in one run.
