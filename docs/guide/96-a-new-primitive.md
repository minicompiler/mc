...
| `lib/f16.mc` | half precision as a **storage** type, with `fcvt` in both directions on **arm64** and a softfloat conversion everywhere else | four slots on a copy of `<float>`'s machine and nothing else, because `<float>` dispatches on `type_kind` and not on the id. The second road needed no machine at all: it registers `f16` as `TK_INT` — two bytes the core moves — and pushes the four names as ordinary functions with `p_push_source`, since a module drives the machines it wrote and no others |
