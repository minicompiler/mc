#!/usr/bin/env python3
"""HTTP micro-benchmark driver. For each server and load configuration:
...
os.chdir(HERE)
SERVERS = {
    "mc-serial":    ["bin/mc-serial"],
    "mc-fork1":     ["bin/mc-fork1"],
    "mc-forkka":    ["bin/mc-forkka"],
    "c-serial":     ["bin/c-serial"],
    "go-nethttp":   ["bin/go-nethttp"],
    "rust-threads": ["bin/rust-threads"],
    "zig-threads":  ["bin/zig-threads"],
    "cs-jit":       ["dotnet", "bin/cs-jit/cs.dll"],
    "cs-aot":       ["bin/cs-aot/cs"],
}
CONFIGS = {
...
PORT0 = int(os.environ.get("PORT0", "18100"))
...
def server_pid(pgid):
    """the server is /usr/bin/time's child: the wrapper exec'd into time, so time's
    pid == the group id; a fork server's workers are the server's children, not time's"""
    r = sh(["ps", "-axo", "pid=,ppid="])
    for line in r.stdout.splitlines():
        pid, ppid = line.split()
        if int(ppid) == pgid:
            return int(pid)
