// sys_linux_aarch64.mc — the Linux system layer on AArch64: the same interface
// as lib/sys.mc with no libc at all, each system call a handful of words taught
// through #opcode. It is lib/sys_svc.mc's sibling — same idea, other kernel
// (M16, docs/build.md § Linux targets); the OS half it includes first is
// lib/sys_linux.mc, and lib/sys_linux_x86_64.mc is the other architecture.
//
...
//
// Every line below the include was lib/sys_linux.mc's, unchanged and in the same
// order, so the object this file compiles to is byte for byte the one that name
...
// x0 already holds it, so nothing has to move. It is reached through a raw
// relocation and NOT by name, which is what lets this file be included into the
// very unit that defines `main` (lib/sys_linux_x86_64.mc explains what x86-64
// has to do instead, and why).
